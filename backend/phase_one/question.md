...
- 密码学：
  ...
  - 哈希函数
    ...
    - 随机性
- 共识机制：
  - 每个区块广播给全网后，所有节点都会验证区块哈希、交易签名、前区块哈希是否匹配，如果验证通过，才会将其加入本地账本，如果某个节点广播了“被篡改的区块”，因为哈希或签名验证不通过，其他节点会拒绝接受，这意味着即使某人本地篡改了账本，全网的共识仍然保持一致，篡改无效
  - 如果攻击者要改某个区块的交易数据，那么就要重新计算该区块的哈希，然后重新计算该区块之后的所有区块的哈希，并且需要构造一个比全网更长的链，使得新的链成为主链，攻击者至少需要全网的51%算力，想改的代价极高，几乎不可能成功
## POW的难度调整机制
- 每产生2016块重新计算难度
